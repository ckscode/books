...
const CreateBook = () => {
    ...
    const validationSchema1 = Yup.object().shape({
        ...
        author: Yup.string().matches(/^[A-Z][a-zA-Z]+(?:['-][A-Z][a-zA-Z]+)*(?: [A-Z][a-zA-Z]+(?:['-][A-Z][a-zA-Z]+)*)*$/,"proper name is required"),
        isbn: Yup.string().matches(/^(?:\d{1,5}[- ]?\d{1,7}[- ]?\d{1,7}[- ]?[\dX]|\d{3}[- ]?\d{1,5}[- ]?\d{1,7}[- ]?\d{1,7}[- ]?\d)$/,"not a valid ISBN").required("ISBN number is required"),
    ...
    const formik1 = useFormik({
        ...
        onSubmit:async (values) =>{
            try{
                await axios.post('https://6642ef123c01a059ea20db85.mockapi.io/api/books',values).then((e)=>console.log(e.data))
                navigate('/')
            }catch(error){
                console.log(error)
            }}
    ...
    return (
        ...
        <h1 className='mb-4 text-center'>Add Book</h1>
          ...
          <button onClick={()=>navigate('/')} className='btn btn-outline-danger ms-3'>Cancel</button>
          </form>
